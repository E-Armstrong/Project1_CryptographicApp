...
public class receiver {
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            // Step 4
            // Get cypher file            
            FileInputStream inputStream = new FileInputStream("message.rsacipher");
            ...
            BufferedInputStream addmsgIN = new BufferedInputStream(new FileInputStream("message.add-msg"));
            byte[] encryptedHash = addmsgIN.readNBytes(32);

            // Copy rest of message from message.add-msg to user named message
            BufferedOutputStream messageOut = new BufferedOutputStream(new FileOutputStream("" + messageFileName));
            while ((addmsgIN.available()) > 0) {
                messageOut.write(addmsgIN.readNBytes(300));
            }
            
/*             BufferedOutputStream messageOut = new BufferedOutputStream(new FileOutputStream("" + messageFileName));
            byte[] piece2 = new byte[300];
            int i;
            while ((i = addmsgIn.read(piece2)) != -1) {
                messageOut.write(addmsgIN.read(Arrays.copyOfRange(piece2, 0, i)));
            } */
            ...
            BufferedInputStream messageBufStream = new BufferedInputStream(new FileInputStream("" + messageFileName));
